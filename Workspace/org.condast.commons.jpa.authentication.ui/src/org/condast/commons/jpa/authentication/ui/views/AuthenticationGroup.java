...
public class AuthenticationGroup extends AbstractEntityGroup<LoginData>
{
	...
	private static final String S_LOGIN_DETAILS = "login Details:";
	...
	private AuthenticationGroup container;
	...
	protected void createComposite(Composite parent, int style) {
		...
		agreementComposite.setLayoutData( new GridData( SWT.FILL, SWT.FILL, true, false ));
		...
		Link forgotLink = new Link( buttons, SWT.NONE);
		forgotLink.setData( RWT.MARKUP_ENABLED, Boolean.TRUE );
		forgotLink.setText("<a>Forgot Password</a>");
		forgotLink.setLayoutData( new GridData( SWT.RIGHT, SWT.FILL, false, true));
		forgotLink.addSelectionListener( new SelectionAdapter() {
			private static final long serialVersionUID = 1L;

			@Override
			public void widgetSelected(SelectionEvent e) {
				try {
					LoginData data = new LoginData( text_nickname.getText(), text_password.getText(), null); 
					notifyInputEdited( new EditEvent<LoginData>( container, EditTypes.CUSTOM, data ));
					super.widgetSelected(e);
				} catch (Exception e1) {
					e1.printStackTrace();
				}
